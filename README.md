...
[![Cases](./assets/badges/cases.svg)](#the-six-sections)
[![Apache 2.0](./assets/badges/license.svg)](./LICENSE)

A signed, cross-language conformance battery for RFC 9421 HTTP Message
Signatures. **11-way byte-for-byte consensus:** eleven independent
implementations, in eleven programming languages, each consuming one frozen,
...
independent implementations concur on it.

Two batteries: **`rfc9421_negative_v1`** (78 cases, the frozen CORE layer) and
**`rfc9421_negative_v2`** (89 cases, a strict superset that adds the fail-closed
signing-base error paths and additional Ed25519/ECDSA range negatives surfaced by
security review). Every verdict is computed from the reference implementation and
frozen; authoritative counts live in each corpus's signed `manifest.json`.

Eleven languages: **python, typescript, go, rust, c, java, kotlin, dotnet, ruby,
php, elixir**. The python/typescript/go/rust runners drive the published AlgoVoi
verifiers; the other seven are self-contained ports of the same verdict surface,
...
runners/{python,typescript,go,rust,c,java,kotlin,dotnet,ruby,php,elixir}/
tools/
...
1. **Agreement** - `tools/run_consensus.sh --require 11` runs all eleven runners
   against the one corpus; because the frozen corpus is the shared oracle,
   all-pass = all eleven agree byte-for-byte. A missing toolchain shrinks N below
...
3. **Cells** - `kaf/run_cells.sh` re-runs each runner inside its pinned Docker
   image, so the verdicts are shown to hold in eleven clean, independent runtimes.
...
```bash
# the whole gate in one command (KAT -> 11-way consensus, fail-closed)
bash tools/run_consensus.sh --require 11
```
...
Eleven independent implementations, each with its own crypto backend, reproduce
every case byte-for-byte (`rfc9421_negative_v1` 78/78 and `rfc9421_negative_v2`
89/89):

| Language | Runner | Ed25519 + key gate | ECDSA P-256/P-384 |
|---|---|---|---|
| Python | published `algovoi-rfc9421-verifier` | PyNaCl / libsodium | `cryptography` |
| TypeScript | published `@algovoi/rfc9421-verifier` | `@noble/ed25519` | `@noble/curves` |
| Go | `algovoi-rfc9421-verifier-go` (in-tree test) | Go stdlib `crypto/ed25519` | Go stdlib `crypto/ecdsa` |
| Rust | `algovoi-rfc9421-verifier-rs` (in-tree test) | `ed25519-dalek` | RustCrypto `p256`/`p384` |
| C | self-contained | OpenSSL EVP + BIGNUM key gate | OpenSSL EC |
| Java | self-contained | Bouncy Castle | JDK `java.security` |
| Kotlin | self-contained | Bouncy Castle | JDK `java.security` |
| .NET | self-contained | Bouncy Castle | `System.Security.Cryptography` |
| Ruby | self-contained | OpenSSL (SPKI-DER) | OpenSSL |
| PHP | self-contained | ext-sodium + ext-gmp | OpenSSL |
| Elixir | self-contained | Erlang `:crypto` | Erlang `:crypto` |
...
hold in eleven clean, independent runtimes rather than only on the build host.
...
fail-open or echoing port. Candidate next stacks: **Scala/Clojure** (JVM),
...
> AlgoVoi RFC 9421 Conformance, <https://github.com/chopmob-cloud/algovoi-rfc9421-conformance>. 78 (v1) / 89 (v2) cases across 6 sections, byte-for-byte reproduced by eleven independent implementations (python, typescript, go, rust, c, java, kotlin, dotnet, ruby, php, elixir), sealed across eleven hermetic runtime cells.

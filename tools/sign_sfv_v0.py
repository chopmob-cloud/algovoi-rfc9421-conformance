#!/usr/bin/env python3
"""Sign + version sfv_v0 with the algovoi-corpus-cm backbone.
...
Override the key with ALGOVOI_SFV_SIGNING_KEY. Manifest/provenance are written
LF-only for a byte-stable signed digest on any platform. Foundation/KMS custody
and publish are gated.

Run:  ALGOVOI_LOCAL_SRC=/path/to/algovoi-corpus-cm/src python tools/sign_sfv_v0.py
"""
...
CORPUS_DIR = os.path.join(REPO, "corpus", "sfv_v0")
...
KEY_ENV = "ALGOVOI_SFV_SIGNING_KEY"
DEFAULT_KEY_PATH = os.path.join(
    os.path.expanduser("~"), ".algovoi", "keys", "sfv-v0.ed25519.json")
...
def load_or_create_key():
    ...
    with open(path, "w", encoding="utf-8") as f:
        json.dump({"seed_hex": seed.hex(), "kid": key.kid, "alg": "Ed25519",
                   "purpose": "sfv_v0 corpus signing (local secret dev key)",
...
def main() -> int:
    ...
    json.dump(manifest, open(os.path.join(CORPUS_DIR, "sfv_v0.manifest.json"), "w", encoding="utf-8", newline="\n"), indent=2)
    json.dump({"issuer_id": log.issuer_id, "records": log.records},
              open(os.path.join(CORPUS_DIR, "sfv_v0.provenance.json"), "w", encoding="utf-8", newline="\n"), indent=2)

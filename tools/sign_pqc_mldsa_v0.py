#!/usr/bin/env python3
"""Sign + version pqc_mldsa_v0 with the algovoi-corpus-cm backbone.
...
Override the key with ALGOVOI_PQC_MLDSA_SIGNING_KEY. Manifest/provenance are
written LF-only for a byte-stable signed digest on any platform. Foundation/KMS
custody and publish are gated.

This signs the corpus HEAD only. It reads nothing but the public corpus file; the
pqc_mldsa_v0 material's private ML-DSA key is not needed here and stays off-repo.

Run:  ALGOVOI_LOCAL_SRC=/path/to/algovoi-corpus-cm/src python tools/sign_pqc_mldsa_v0.py
"""
...
CORPUS_DIR = os.path.join(REPO, "corpus", "pqc_mldsa_v0")
...
# Only the sections present in stage-1; mldsa65_acvp_kat is a later addition and
# is signed automatically once the generator emits it (guarded by .get below).
SECTIONS = ("mldsa65_verify", "mldsa65_malformed", "mldsa65_acvp_kat")

KEY_ENV = "ALGOVOI_PQC_MLDSA_SIGNING_KEY"
DEFAULT_KEY_PATH = os.path.join(
    os.path.expanduser("~"), ".algovoi", "keys", "pqc-mldsa-v0.ed25519.json")
...
def load_or_create_key():
    ...
    with open(path, "w", encoding="utf-8") as f:
        json.dump({"seed_hex": seed.hex(), "kid": key.kid, "alg": "Ed25519",
                   "purpose": "pqc_mldsa_v0 corpus signing (local secret dev key)",
...
def build_head(corpus: dict, raw_bytes: bytes) -> dict:
    present = [s for s in SECTIONS if isinstance(corpus.get(s), list)]
    return {
        ...
        "section_counts": {s: len(corpus[s]) for s in present},
        "section_digests": {s: digest(corpus[s]) for s in present},
        "total_cases": sum(len(corpus[s]) for s in present),
    }
...
def main() -> int:
    ...
    json.dump(manifest, open(os.path.join(CORPUS_DIR, "pqc_mldsa_v0.manifest.json"), "w", encoding="utf-8", newline="\n"), indent=2)
    json.dump({"issuer_id": log.issuer_id, "records": log.records},
              open(os.path.join(CORPUS_DIR, "pqc_mldsa_v0.provenance.json"), "w", encoding="utf-8", newline="\n"), indent=2)

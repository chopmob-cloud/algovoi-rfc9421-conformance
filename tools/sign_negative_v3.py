#!/usr/bin/env python3
"""Sign + version rfc9421_negative_v3 with the algovoi-corpus-cm backbone.

Same trust model as sign_negative_v1/v2.py: a compact CORPUS HEAD is EdDSA-signed
with a genuine SECRET Ed25519 seed held OFF-REPO; only the public JWK ships. v3
signs the seven sections (the v2 six plus rsa_verify). Override the key with
ALGOVOI_NV3_SIGNING_KEY. Foundation/KMS custody and publish are gated.

Run:  python tools/sign_negative_v3.py
"""
...
CORPUS_DIR = os.path.join(REPO, "corpus", "rfc9421_negative_v3")
CORPUS = os.path.join(CORPUS_DIR, "rfc9421_negative_v3.json")

CORPUS_ID = "rfc9421_negative_v3"
VERSION = "0.3.0"
...
SECTIONS = ("signing_base", "signature_input_parse", "signature_value_parse",
            "keygate", "ed25519_verify", "ecdsa_verify", "rsa_verify")

KEY_ENV = "ALGOVOI_NV3_SIGNING_KEY"
DEFAULT_KEY_PATH = os.path.join(
    os.path.expanduser("~"), ".algovoi", "keys", "rfc9421-negative-v3.ed25519.json")
...
def load_or_create_key():
    ...
    with open(path, "w", encoding="utf-8") as f:
        json.dump({"seed_hex": seed.hex(), "kid": key.kid, "alg": "Ed25519",
                   "purpose": "rfc9421_negative_v3 corpus signing (local secret dev key)",
...
def main() -> int:
    ...
    json.dump(manifest, open(os.path.join(CORPUS_DIR, "rfc9421_negative_v3.manifest.json"), "w", encoding="utf-8"), indent=2)
    json.dump({"issuer_id": log.issuer_id, "records": log.records},
              open(os.path.join(CORPUS_DIR, "rfc9421_negative_v3.provenance.json"), "w", encoding="utf-8"), indent=2)

#!/usr/bin/env python3
"""Sign + version webbotauth_v0 with the algovoi-corpus-cm backbone.
...
Override the key with ALGOVOI_WBA_SIGNING_KEY. Manifest/provenance are written
LF-only for a byte-stable signed digest on any platform. Foundation/KMS custody
and publish are gated.

Run:  ALGOVOI_LOCAL_SRC=/path/to/algovoi-corpus-cm/src python tools/sign_webbotauth_v0.py
"""
...
CORPUS_DIR = os.path.join(REPO, "corpus", "webbotauth_v0")
CORPUS = os.path.join(CORPUS_DIR, "webbotauth_v0.json")

CORPUS_ID = "webbotauth_v0"
VERSION = "0.1.0"
CANONICALIZER = "JCS(RFC8785)+EdDSA"
HEAD_TYP = "corpus-head+jws"
TS = "2026-08-12T00:00:00Z"
SECTIONS = ("wba_signing_base", "wba_coverage", "wba_freshness",
            "wba_directory", "wba_directory_ssrf", "wba_ed25519_verify")

KEY_ENV = "ALGOVOI_WBA_SIGNING_KEY"
DEFAULT_KEY_PATH = os.path.join(
    os.path.expanduser("~"), ".algovoi", "keys", "webbotauth-v0.ed25519.json")
...
def load_or_create_key():
    ...
    with open(path, "w", encoding="utf-8") as f:
        json.dump({"seed_hex": seed.hex(), "kid": key.kid, "alg": "Ed25519",
                   "purpose": "webbotauth_v0 corpus signing (local secret dev key)",
...
def main() -> int:
    ...
    json.dump(manifest, open(os.path.join(CORPUS_DIR, "webbotauth_v0.manifest.json"), "w", encoding="utf-8", newline="\n"), indent=2)
    json.dump({"issuer_id": log.issuer_id, "records": log.records},
              open(os.path.join(CORPUS_DIR, "webbotauth_v0.provenance.json"), "w", encoding="utf-8", newline="\n"), indent=2)

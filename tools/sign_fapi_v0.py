#!/usr/bin/env python3
"""Sign + version fapi_messagesigning_v0 with the algovoi-corpus-cm backbone.

Same trust model as sign_negative_v3.py: a compact CORPUS HEAD is EdDSA-signed
with a genuine SECRET Ed25519 seed held OFF-REPO; only the public JWK ships.
Override the key with ALGOVOI_FAPI_SIGNING_KEY. Manifest/provenance are written
LF-only for a byte-stable signed digest on any platform. Foundation/KMS custody
and publish are gated.

Run:  ALGOVOI_LOCAL_SRC=/path/to/algovoi-corpus-cm/src python tools/sign_fapi_messagesigning_v0.py
"""
...
CORPUS_DIR = os.path.join(REPO, "corpus", "fapi_messagesigning_v0")
CORPUS = os.path.join(CORPUS_DIR, "fapi_messagesigning_v0.json")

CORPUS_ID = "fapi_messagesigning_v0"
VERSION = "0.1.0"
CANONICALIZER = "JCS(RFC8785)+EdDSA"
HEAD_TYP = "corpus-head+jws"
TS = "2026-08-12T00:00:00Z"
SECTIONS = ("fapi_signing_base", "fapi_required_coverage", "fapi_content_digest",
            "fapi_alg", "fapi_ps256_verify", "fapi_es256_verify")

KEY_ENV = "ALGOVOI_FAPI_SIGNING_KEY"
DEFAULT_KEY_PATH = os.path.join(
    os.path.expanduser("~"), ".algovoi", "keys", "fapi-messagesigning-v0.ed25519.json")
...
def load_or_create_key():
    ...
    with open(path, "w", encoding="utf-8") as f:
        json.dump({"seed_hex": seed.hex(), "kid": key.kid, "alg": "Ed25519",
                   "purpose": "fapi_messagesigning_v0 corpus signing (local secret dev key)",
...
def main() -> int:
    ...
    json.dump(manifest, open(os.path.join(CORPUS_DIR, "fapi_messagesigning_v0.manifest.json"), "w", encoding="utf-8", newline="\n"), indent=2)
    json.dump({"issuer_id": log.issuer_id, "records": log.records},
              open(os.path.join(CORPUS_DIR, "fapi_messagesigning_v0.provenance.json"), "w", encoding="utf-8", newline="\n"), indent=2)

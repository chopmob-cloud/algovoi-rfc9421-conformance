#!/usr/bin/env python3
"""Seal a KAF assurance receipt for the FAPI 2.0 Message Signing profile (fapi_messagesigning_v0).
...
Binds the KAF axes this profile proves into one signed, re-verifiable receipt:
  ...
  - KAT       : the corpus is the exact signed artifact and every verdict is
                independently re-derivable (tools/check_kat_fapi.py).
  - Cells     : per-runtime verdicts from kaf/run_cells_wba.sh (image digests).
  - Seal      : this receipt, EdDSA-signed over the canonical (RFC 8785 JCS)
                payload, bound to the corpus's signed manifest head.

Uses the same rfc9421 KAF seal identity as the negative-battery receipts (the seal
secret is an Ed25519 key held OFF-REPO; only the public JWK ships). Refuses to seal
unless the KAT gate passes and every cell passed.

Usage:
  python kaf/seal_receipt_wba.py --secret <path> --sealed-at <iso8601> [--seq N]
"""
...
def main() -> int:
    ...
    ap.add_argument("--corpus", default=os.path.join(
        HERE, "corpus", "fapi_messagesigning_v0", "fapi_messagesigning_v0.json"))
    ap.add_argument("--cells", default=os.path.join(HERE, "kaf", "cells_fapi.results.json"))
    args = ap.parse_args()
    ...
    kat = subprocess.run([sys.executable, os.path.join(HERE, "tools", "check_kat_fapi.py"), corpus_path],
    ...
    payload = {
        "framework": "KAF",
        "layer": "L2",
        "profile": corpus.get("profile", "fapi-2-message-signing"),
        "suite": corpus["name"],

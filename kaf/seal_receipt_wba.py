#!/usr/bin/env python3
"""Seal a KAF assurance receipt for the Web Bot Auth profile (webbotauth_v0).
...
Binds the KAF axes this profile proves into one signed, re-verifiable receipt:
  ...
  - KAT       : the corpus is the exact signed artifact and every verdict is
                independently re-derivable (tools/check_kat_wba.py).
...
def main() -> int:
    ...
    ap.add_argument("--corpus", default=os.path.join(
        HERE, "corpus", "webbotauth_v0", "webbotauth_v0.json"))
    ap.add_argument("--cells", default=os.path.join(HERE, "kaf", "cells_wba.results.json"))
    args = ap.parse_args()
    ...
    kat = subprocess.run([sys.executable, os.path.join(HERE, "tools", "check_kat_wba.py"), corpus_path],
    ...
    payload = {
        "framework": "KAF",
        "layer": "L2",
        "profile": corpus.get("profile", "web-bot-auth"),
        "suite": corpus["name"],

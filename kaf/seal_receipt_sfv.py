#!/usr/bin/env python3
"""Seal a KAF assurance receipt for the Structured Field Values profile (sfv_v0).
...
Binds the KAF axes this profile proves into one signed, re-verifiable receipt:
  ...
  - Agreement : the N-way byte-for-byte consensus, DERIVED from the distinct
                languages whose hermetic cells actually passed.
  - KAT       : the corpus is the exact signed artifact and every verdict is
                independently re-derivable with a separate RFC 8941 implementation
                (tools/check_kat_sfv.py, http_sfv).
  - Cells     : per-runtime verdicts from kaf/run_cells_sfv.sh (image digests).
  - Seal      : this receipt, EdDSA-signed over the canonical (RFC 8785 JCS)
                payload, bound to the corpus's signed manifest head.

Uses the same rfc9421 KAF seal identity as the negative-battery and profile
receipts (the seal secret is an Ed25519 key held OFF-REPO; only the public JWK
ships). Refuses to seal unless the KAT gate passes and every cell passed.

Usage:
  python kaf/seal_receipt_sfv.py --secret <path> --sealed-at <iso8601> [--seq N]
"""
...
def main() -> int:
    ...
    ap.add_argument("--corpus", default=os.path.join(
        HERE, "corpus", "sfv_v0", "sfv_v0.json"))
    ap.add_argument("--cells", default=os.path.join(HERE, "kaf", "cells_sfv.results.json"))
    args = ap.parse_args()
    ...
    kat = subprocess.run([sys.executable, os.path.join(HERE, "tools", "check_kat_sfv.py"), corpus_path],
    ...
    payload = {
        "framework": "KAF",
        "layer": "L2",
        "profile": corpus.get("profile", "structured-field-values"),
        "suite": corpus["name"],
        ...
        "axes": {
            ...
            "kat": {"corpus_integrity": "matches signed manifest head",
                    "independent_rederivation": "every verdict re-derived independently"},
            "cells": [

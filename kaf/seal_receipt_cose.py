#!/usr/bin/env python3
"""Seal a KAF assurance receipt for the COSE_Sign1 profile (cose_v0).
...
Binds the KAF axes this profile proves into one signed, re-verifiable receipt:
  ...
  - KAT       : the corpus is the exact signed artifact and every verdict is
                independently re-derivable (tools/check_kat_cose.py).
  - Cells     : per-runtime verdicts from kaf/run_cells_cose.sh (image digests).
...
Usage:
  python kaf/seal_receipt_cose.py --secret <path> --sealed-at <iso8601> [--seq N]
"""
...
def main() -> int:
    ...
    ap.add_argument("--corpus", default=os.path.join(HERE, "corpus", "cose_v0", "cose_v0.json"))
    ap.add_argument("--cells", default=os.path.join(HERE, "kaf", "cells_cose.results.json"))
    args = ap.parse_args()
    ...
    kat = subprocess.run([sys.executable, os.path.join(HERE, "tools", "check_kat_cose.py"), corpus_path],
    ...
    payload = {
        "framework": "KAF",
        "layer": "L2",
        "profile": corpus.get("profile", "cose-sign1"),
        "suite": corpus["name"],

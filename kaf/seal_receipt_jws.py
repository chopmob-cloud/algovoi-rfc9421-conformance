#!/usr/bin/env python3
"""Seal a KAF assurance receipt for the JSON Web Signature profile (jws_v0).
...
Binds the KAF axes this profile proves into one signed, re-verifiable receipt:
  ...
  - KAT       : the corpus is the exact signed artifact and every verdict is
                independently re-derivable (tools/check_kat_jws.py).
  - Cells     : per-runtime verdicts from kaf/run_cells_jws.sh (image digests).
...
Usage:
  python kaf/seal_receipt_jws.py --secret <path> --sealed-at <iso8601> [--seq N]
"""
...
def main() -> int:
    ...
    ap.add_argument("--corpus", default=os.path.join(HERE, "corpus", "jws_v0", "jws_v0.json"))
    ap.add_argument("--cells", default=os.path.join(HERE, "kaf", "cells_jws.results.json"))
    args = ap.parse_args()
    ...
    kat = subprocess.run([sys.executable, os.path.join(HERE, "tools", "check_kat_jws.py"), corpus_path],
    ...
    payload = {
        "framework": "KAF",
        "layer": "L2",
        "profile": corpus.get("profile", "json-web-signature"),
        "suite": corpus["name"],
